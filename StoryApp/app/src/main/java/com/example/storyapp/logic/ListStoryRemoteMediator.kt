...
import com.example.storyapp.logic.repository.db.RemoteKeys
...
class ListStoryRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            val _token = pref.getUser().token.toString()
            val token = "Bearer $_token"
            val responseData = apiService.getListStory2(token,page, state.config.pageSize).listStory
