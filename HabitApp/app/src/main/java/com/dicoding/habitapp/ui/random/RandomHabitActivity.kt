...
class RandomHabitActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.priorityLevelHigh.observe(this) {
            adapter.submitData(RandomHabitAdapter.PageType.HIGH, it ?: Habit(title = "No habit found", minutesFocus = 0, startTime = "00:00", priorityLevel = "High"))
        }
        viewModel.priorityLevelMedium.observe(this) {
            adapter.submitData(RandomHabitAdapter.PageType.MEDIUM, it ?: Habit(title = "No habit found", minutesFocus = 0, startTime = "00:00", priorityLevel = "Medium"))
        }
        viewModel.priorityLevelLow.observe(this) {
            adapter.submitData(RandomHabitAdapter.PageType.LOW, it ?: Habit(title = "No habit found", minutesFocus = 0, startTime = "00:00", priorityLevel = "Low"))
        }
